...
import { CATEGORY_TRANSLATIONS, CONTENT_TRANSLATIONS } from '../i18n';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const content = CONTENT_TRANSLATIONS[lang] || CONTENT_TRANSLATIONS['en'];
  const descriptions = content.nodes;
  const nodeNames = content.nodeNames;
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 w-80 bg-[#0F1116] border-r border-slate-800 z-50 transform transition-transform duration-300 md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col shadow-2xl h-full max-h-[100dvh]`}>
      {/* Header - Fixed */}
      <div className="p-4 border-b border-slate-800/50 flex items-center justify-between shrink-0 bg-[#0F1116]">
         <div className="font-bold text-white flex items-center gap-2.5 text-lg tracking-tight">
           <div className="bg-gradient-to-br from-indigo-600 to-violet-600 p-1.5 rounded-lg shadow-lg shadow-indigo-500/20"><Zap size={18} className="text-white"/></div>
           AutoFlow <span className="text-indigo-500">Pro</span>
         </div>
         <button className="md:hidden text-slate-400 hover:text-white p-2" onClick={() => setIsMobileMenuOpen(false)}><X size={24}/></button>
      </div>
         ...
         <div className="space-y-1 pb-20"> {/* Extra padding bottom for mobile ease */}
           {categoryOrder.map(catKey => {
             const nodesInCat = NODE_CATALOG.filter(n => 
               n.category === catKey && (n.name.toLowerCase().includes(searchQuery.toLowerCase()) || nodeNames[n.type]?.toLowerCase().includes(searchQuery.toLowerCase()))
             );
                     ...
                     {nodesInCat.map(item => (
                       ...
                       >
                          ...
                          <div className="overflow-hidden min-w-0">
                             {/* Use Translated Node Name */}
                             <div className="text-sm font-medium text-slate-300 group-hover:text-white truncate">{nodeNames[item.type] || item.name}</div>
                             <div className="text-[10px] text-slate-500 truncate">{descriptions[item.type] || item.desc}</div>
                          </div>
